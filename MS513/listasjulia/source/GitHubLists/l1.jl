...
`Data de entrega`: **DD/MM, 2024 às HH:mm**
...
function demean(vector)
	...
	vector_copy = vector_copy .- mean(vector_copy)
    return vector_copy
end
...
👉 Escreva uma função que cria um vetor de 100 elementos em que:
...
apolo_filename = "C:\\Users\\reisd\\Documents\\Undergrad\\MS513\\listasjulia\\wallstranding2.jpg" # download to a local file. The filename is returned
...
# ╔═╡ 0a93927d-6f18-44f0-851b-32d77e5707cb
apolo_height = size(apolo)[1]

# ╔═╡ 732260b1-a888-42df-b4ee-b285dd82b081
apolo_width = size(apolo)[2]

# ╔═╡ 77b3b940-0e00-4311-8a41-0ff90cd31144
@bind range_y RangeSlider(1:apolo_height)

# ╔═╡ 4a48603a-5685-4140-a812-8ebadd1f012d
@bind range_x RangeSlider(1:apolo_width)

# ╔═╡ 6ccd8902-0dd9-4335-a11a-ee7f9a1a6c09
apolo_head = apolo[range_y, range_x]
...
x=get_red(RGB(0.8, 0.1, 0.0))

# ╔═╡ 64f9bdf4-745d-4e92-aecd-bc8477ee7e20
typeof(x)
...
function get_reds(image::AbstractMatrix)
    r_intensity_mask = get_red.(image)
    return r_intensity_mask
end
...
# ╔═╡ ec923a70-7cb6-46d8-87cd-636a3a91fa8b
red_apolo = value_as_color.(get_reds(apolo_head))
...
# ╔═╡ d3799429-cb26-4fd0-ab34-5ac91eefba85
function get_greens(image::AbstractMatrix)
    g_intensity_mask = get_green.(image)
    return g_intensity_mask
end

# ╔═╡ e83a28fb-4588-4507-a2bc-a71f68f238fa
function value_as_green(x)

    return RGB(0, x, 0)
end

# ╔═╡ c8ecebe1-7287-40c8-85de-18992fa0a1cd
function get_blue(pixel::AbstractRGB)
	blue = pixel.b
	return blue
end

# ╔═╡ 93e04b88-a554-429c-bdad-6187cfd4d99f
function get_blues(image::AbstractMatrix)
    b_intensity_mask = get_blue.(image)
    return b_intensity_mask
end

# ╔═╡ dce3cd9e-0425-4b21-9c0e-0177c2ce7d0b
function value_as_blue(x)

    return RGB(0, 0, x)
end

# ╔═╡ 8c7fd43a-3835-4707-aadd-e1b854b5b94a
green_apolo = value_as_green.(get_blues(copy(apolo_head)))

# ╔═╡ 2d518ceb-b818-4ac6-b8a6-f8ee3c2df812
blue_apolo = value_as_blue.(get_blues(copy(apolo_head)))
...
function invert(color::AbstractRGB)
	cor = RGB(1 - color.r, 1 - color.g, 1 - color.b)
    return cor
end
...
apolo_inverted = invert.(apolo) # replace `missing` with your code!
...
function quantize(color::AbstractRGB)
    cor = RGB(quantize(color.r), quantize(color.g), quantize(color.b))
    return cor
end
...
function noisify(color::AbstractRGB, s)
    cor = RGB(noisify(color.r, s), noisify(color.g, s), noisify(color.b, s))
    return cor
end
...
A imagem se torna irreconhecível a partir de 85% de intensidade, sendo presevada apenas a silhueta de regiões grandes com alto contraste. Com 95%, mesmo essa informação parece se perder e vemos praticamente apenas ruído.
"""
...
function custom_filter(pixel::AbstractRGB)
    ...
    pixel = RGB(1 - pixel.r*3, 1 - pixel.g*3, 1 - pixel.b*3)
...
manifest_format = "2.0"
project_hash = "12740a5ba59a41a4e44fea6f16295ebc098e2114"
