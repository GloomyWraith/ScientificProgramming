...
* [Documentação oficial](https://docs.juliaplots.org/stable/tutorial/#tutorial)
* [Exemplos](https://docs.juliaplots.org/stable/gallery/gr/)
...
A inicialização da `Plots` é um pouco mais demorada do que, por exemplo, a da `matplotlib` de Python. Já foi muito pior, mas as últimas versões de Julia atenuaram esse problema a ponto de ele não der mais desagradável. Tudo isso preservando a velocidade de Julia depois que compila o código pela primeira vez.
"""
...
md"""Note que ela já tenta fazer múltiplas coisas para você, talvez um pouco mais do outras bibliotecas gráficas. O objetivo é já gerar um gráfico "pronto". Um exemplo é a legenda `y1` que é adicionada aumaticamente e que ajuda a saber qual foi a primeira função a ser desenhada. Você pode mudar o texto da legenda passando um parâmetro extra para a `plot` chamado de `label`. Se quiser omitir a lengeda passe para `label` o valor `false` ou a string vazia (`""`).
"""
...
Outra novidade na figura acima é o uso da função do tipo `scatter`. Essa função ao invés de desenhar o gráfico de uma função, imaginando que os pontos repassados estão ligados, considera que devem apenas ser desenhados os pontos isolados, nesse caso usando o símbolo do círculo. Note como na função `plot`,  ao repassar apenas um vetor, ela assume que ele possui os valores da coordenada y e para a coordenada x usa um vetor que começa em 1 e vai até o número de elementos do vetor y pulando de um em um."
...
md"""
Note o uso do parâmtro `yflip` acima. Tipicamente o heatmap tem o eixo y crescendo para cima e em figuras isso é tipicamente o que você quer. Mas se você quer vez uma matriz matemática o "eixo y" (que é o número da linha da matriz) cresce para baixo! Então invertemos a ordem do eixo y usando o parâmetro `yflip` para evitar que a imagem da matriz pareça invertida.
"""
...
md"""
De fato `Plots` é um pacote bastante geral que possui muitas funcinalidade e várias extensões. Despois dessa rápida introdução simples vá para o [tutorial oficial](https://docs.juliaplots.org/stable/tutorial/) e aprenda um ponto mais.
"""
...
manifest_format = "2.0"
project_hash = "9fba97e2c706ca7d17a62fba3933803fbfd3b6ff"
